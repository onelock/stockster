...
class StockDataLoader:
    ...
    def load_latest_stocks(self, limit: int = 100) -> pd.DataFrame:
        ...
        try:
            response = requests.get(
                # f"{self.api_url}/api/v1/alpaca/bars/latest?limit={limit}", 
                f"{self.api_url}/api/v1/alpaca/bars/latest", 
                timeout=10
            )
            ...
            df.rename(columns={'symbol':'name', 'open': 'last_price'}, inplace=True)
            print(df.columns)
            df['change_pct']
    ...
    #     try:
    #         response = requests.get(
    #             f"{self.api_url}/api/v1/alpaca/bars/{stock_symbol}?days={days}", 
    #             timeout=10
    #         )
    #         response.raise_for_status()
    #         data = response.json()
    ...
    #     try:
    #         response = requests.get(f"{self.api_url}/api/v1/alpaca/stats", timeout=5)
    #         response.raise_for_status()
